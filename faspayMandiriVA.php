...
function faspayMandiriVA_config() {
	$configarray = array(
    "FriendlyName" => array("Type" => "System", "Value"=>"Mandiri VA"),
        ...
        "faspay_channel" => array("FriendlyName" => "Channel Code", "Type" => "text", "Size" => "50", "Value"=>"802"),
...
function faspayMandiriVA_link($params) {
    ...
    $VANumber = getVAMandiriNumber($bill_no);
    ...
    if($VANumber == 'Virtual Account Number :' OR $VANumber == '') {
        ...
        '</faspay>';        
        $data_faspay = faspay_postxmlMandiriVA($faspay_post_act,$faspay_post_trans);
		
        try{
            if ($data_faspay->response_code == 00) { //sukses
                addVAMandiriNumber($data_faspay->bill_no,$data_faspay->trx_id);
...
function faspay_postxmlMandiriVA($faspay_post_act,$faspay_post_trans) {
...
function addVAMandiriNumber($invoiceid,$va_number) {
...
function getVAMandiriNumber($invoiceid) {
