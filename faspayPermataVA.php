...
function faspayPermataVA_config() {
	$configarray = array(
    "FriendlyName" => array("Type" => "System", "Value"=>"Permata VA"),
        ...
        "faspay_channel" => array("FriendlyName" => "Channel Code", "Type" => "text", "Size" => "50", "Value"=>"402"),
...
function faspayPermataVA_link($params) {
    ...
    $VANumber = getVAPermataNumber($bill_no);
    ...
    if($VANumber == 'Virtual Account Number :' OR $VANumber == '') {
        ...
        '</faspay>';        
        $data_faspay = faspay_postxmlPermataVA($faspay_post_act,$faspay_post_trans);

        try{
            if ($data_faspay->response_code == 00) { //sukses
                addVAPermataNumber($data_faspay->bill_no,$data_faspay->trx_id);
...
function faspay_postxmlPermataVA($faspay_post_act,$faspay_post_trans) {
...
function addVAPermataNumber($invoiceid,$va_number) {
...
function getVAPermataNumber($invoiceid) {
